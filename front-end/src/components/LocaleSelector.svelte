<script>
  ...
  */
  function handleNav(event) {
    ...
    if (newIndex != index) {
      ...
      locale = filteredLocales[newIndex];
  ...
  */
  ...
  function handleKeyboard(event) {
    ...
    if (!open && (event.key == " " || event.key == "Enter")) {
      open = true
    }
  }
...
<Button id="locale-selector" on:keydown={handleKeyboard} class="p-2 mt-1 mb-2 h-10 border-0 rounded-md bg-black/80 hover:bg-black/60">
...
<Dropdown class="overflow-y-auto px-3 pb-3 text-sm h-60" containerClass="divide-y" bind:open>
  <div slot="header" class="p-3">
    <Search size="sm" bind:value={term} autofocus on:keydown={handleNav} />
  </div>
