...
<!-- <div> -->
<!--   <Select defaultClass="p-2 mt-1 mb-2 h-10 border-0 rounded-md bg-black/80" items={localeOptions} bind:value={locale} /> -->
  <!-- </div> -->
...
<Button class="p-2 mt-1 mb-2 h-10 border-0 rounded-md bg-black/80 hover:bg-black/60">
  ...
  <Dropdown class="overflow-y-auto px-3 pb-3 text-sm h-60" containerClass="divide-y" bind:open>
    <div slot="header" class="p-3">
      <Search size="sm" bind:value={term} />
    </div>
    {#each filteredLocales as loc}
      <DropdownItem on:click={() => selectLocale(loc)} defaultClass="text-gray-500">
        <li class="flex rounded p-2 hover:bg-gray-100 dark:hover:bg-gray-600 cursor-pointer">
